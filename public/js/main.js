document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchProfile() {
        try {
            ...
            if (profile.bio) document.querySelector('.hero-content .description').textContent = profile.bio.split('.')[0] + '.'; // Short bio for hero

            // About Section
            if (profile.bio) document.querySelector('.about-text p').textContent = profile.bio;
        ...
        projects.forEach(project => {
            ...
            card.innerHTML = `
                <div class="project-image">
                    <img src="${project.image_url || project.image}" alt="${project.title}" loading="lazy" onerror="this.style.display='none'">
                </div>
                <div class="project-content">
                    ...
                    <div class="project-links">
                        <a href="${project.github_link || project.github}" target="_blank" title="View Code"><i class="fab fa-github"></i></a>
                        ${(project.live_link || project.live) && (project.live_link !== '#' && project.live !== '#') ? `<a href="${project.live_link || project.live}" target="_blank" title="Live Demo"><i class="fas fa-external-link-alt"></i></a>` : ''}
                    </div>
                </div>
            `;
